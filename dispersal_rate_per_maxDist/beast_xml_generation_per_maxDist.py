...
for folder in sorted(os.listdir(base_dir)):
    if folder.startswith("dist") and "_sim" in folder and os.path.isdir(folder):
        ...
        if not fa_files:
            print(f"[SKIP] No FASTA file found in {folder}")
        else:
            xml_file = f"{folder}.xml"
            if os.path.exists(xml_file):
                print(f"[SKIP] {xml_file} already exists.")
            else:
                print(f"[OK] Generating {xml_file} in {folder}")
                generate_beast_xml(output_xml=xml_file)
